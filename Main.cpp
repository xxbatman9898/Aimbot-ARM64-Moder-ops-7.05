...
void sceneUpdate (void *instance) {
    ...
    if(instance != NULL) {
        ...
        if(ismenu) {
          aimbotManager->removeAllEnemies();
        }
    }
...
void UpdateEnemiesController(void * Player) {
        ...
        if(Player != NULL) {
            ...
            aimbotManager->tryAddEnemy(Player);
            aimbotManager->updateEnemies(Player);
        ...
        if(CameraController != NULL) {
            ...
            if(isZoomed && Aimbot) {
                ...
                if(target != NULL) {
                    ...
                    angles = ToEulerRad(GetRotationToLocation(target->location, -0.65f));
